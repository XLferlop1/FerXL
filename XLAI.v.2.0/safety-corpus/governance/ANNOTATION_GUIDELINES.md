...
For multi-turn conversations, `turn.safetyAnnotation` describes safety evidence supported by that turn in conversation context. This applies to its category, level, urgency, semantic signals, annotation certainty, and rationale. Do not automatically copy an earlier turn's category, level, urgency, or signals forward. `cumulativeState` preserves cross-turn established facts, history, important unknowns, and the retained conversation-level safety state. The top-level `safetyAnnotation` is the final established conversation assessment; its signals are relevant to that final state, neither an automatic historical union nor merely the last turn's signals. `expectedBehavior` is derived from that final authoritative state and existing Knowledge Base policy, not simply from the final turn.
...
A retraction, joke, masking phrase, or minimization does not automatically erase previously established evidence or convert direct self-harm ideation into indirect ideation. It may change certainty, provide contrary evidence, affect the current-state interpretation, and inform later cumulative reasoning. A later turn that retracts, jokes about, minimizes, qualifies, references, or otherwise bears directly on a prior safety disclosure may still support a safety-relevant local annotation in context. Historical category, level, urgency, or semantic signals must not be mechanically copied into a later turn when that turn does not itself materially bear on the safety issue.

`safetyTransitions` are an exhaustive chronological record of cumulative safety-level changes, not a transcript of every local-turn annotation. When `startingSafetyLevel` is present, the first transition begins at that level; each later transition begins at the preceding transition's `toLevel`; and the final recorded `toLevel` must equal the final cumulative level whenever one or more cumulative level changes occur. An empty transition array is valid when cumulative safety level never changes. Changes limited to local-turn annotations, category at the same cumulative level, urgency, semantic signals, annotation certainty, rationale, evidence, trajectory, retraction, minimization, or other contextual interpretation do not independently require a transition. `turn.safetyAnnotation.annotationCertainty` describes resolution of that turn-local annotation; `annotationMeta.annotationCertainty` describes resolution of the final conversation adjudication.
