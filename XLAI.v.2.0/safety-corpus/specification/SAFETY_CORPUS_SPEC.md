...
`turn.safetyAnnotation` describes safety evidence supported by that turn in conversation context, including its category, level, urgency, semantic signals, annotation certainty, and rationale. It is not a cumulative-history container. `cumulativeState` preserves established cross-turn facts, history, unknowns, and the retained conversation-level safety state. The top-level safety annotation represents the final established conversation assessment; when it carries semantic signals, they are relevant to that final state rather than only the final turn or an automatic union of all historical evidence. `expectedBehavior` is derived from the top-level final state and existing Knowledge Base policy, not simply from the last turn.

Retraction, joking, masking, and minimization are contextual evidence that can change certainty or later interpretation, but they do not automatically erase established evidence or convert explicit ideation into indirect ideation. A later turn that retracts, jokes about, minimizes, qualifies, references, or otherwise bears directly on a prior safety disclosure may still support a safety-relevant local annotation in context. Historical category, level, urgency, or semantic signals must not be mechanically copied when the later turn does not itself materially bear on that safety issue. `safetyTransitions` are an exhaustive chronological record of cumulative safety-level changes, not every local-turn annotation: the first transition begins from `startingSafetyLevel` when present, each later transition continues from the previous `toLevel`, and the final recorded `toLevel` equals the final cumulative level whenever one or more cumulative level changes occur. An empty transition array is valid when cumulative safety level never changes. Local-turn changes, same-level category changes, urgency, signals, certainty, rationale, evidence, trajectory, retraction, minimization, and other contextual interpretation do not independently require a transition. Turn annotation certainty is local, while `annotationMeta.annotationCertainty` resolves the final conversation adjudication. Concrete preparation toward self-harm or suicide remains deferred: the current corpus has no independently adjudicated preparation examples, and no preparation signal is introduced in this phase.
